...
static int handle_single_conversion(ad7616_dev *dev, const struct spi_read_args *sargs)
{
	...
	if (ad7616_write(dev, AD7616_REG_CHANNEL, vchannel_mask) < 0) {
...
static int handle_burst_conversion(ad7616_dev *dev, const struct spi_read_args *sargs)
{
	...
	if (ad7616_write(dev, AD7616_REG_CONFIG, BURST_EN) < 0) {
...
static int handle_self_test(ad7616_dev *dev, struct spi_read_args *sargs)
{
	...
	if (ad7616_write(dev, AD7616_REG_CHANNEL, diag_chan) < 0) {
