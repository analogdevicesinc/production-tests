...
CMD="echo heartbeat | tee /sys/class/leds/led0:red/trigger > /dev/null;"
CMD+="sleep 0.1 && echo heartbeat | tee /sys/class/leds/led1:red/trigger > /dev/null;"
CMD+="sleep 0.1 && echo heartbeat | tee /sys/class/leds/led2:red/trigger > /dev/null;"
CMD+="sleep 0.1 && echo heartbeat | tee /sys/class/leds/led3:red/trigger > /dev/null;"
CMD+="YES_no 'Are LEDs 0-3 blinking ? '"
run_test $TEST_ID "$SHORT_DESC" "$CMD"

echo none | tee /sys/class/leds/led[0-3]\:red/trigger > /dev/null
