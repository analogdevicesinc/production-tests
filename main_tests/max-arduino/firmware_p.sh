...
## This script is used to flash the firmware to max-arduino board using the DAPLink interface. 
## The apard32690_rev_e.hex file must be downloaded manually in the specified path. 
...
inotifywait -m -e unmount "$mountpoint" | (
    rsync -ahv --progress /home/analog/production-tests/main_tests/max-arduino/apard32690_rev_e.hex "$mountpoint"
    sync
