...
${TOOLCHAIN}gdb --write $SCRIPT_DIR/firmware.elf -ex "set OD_PERSIST_COMM.x1018_identity.serialNumber = ${SERIALNUM}" -ex "q"
...
openocd \
        ...
        -c 'targets' \
        -c 'reset init' \
        -c "flash write_image erase \"$SCRIPT_DIR/firmware.elf\"" \
        -c 'reset run' \
        -c 'shutdown'
