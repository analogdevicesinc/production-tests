...
int main(int argc, char *argv[]) {
	...
	if (strcmp("FMC_GPIO0", argv[1]) == 0) {
		...
		printf("Usage: %s [test_dev FMC_GPIO0]\n", argv[0]);
		return -1;
	}
	...
	void* mem = mmap(NULL, page_offset + GPIO_TRI_REG(CHANNEL_1), PROT_READ | PROT_WRITE,
	...
	close(fd);
	munmap(mem, page_offset + GPIO_TRI_REG(CHANNEL_1));
