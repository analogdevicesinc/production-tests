...
wait_for_board_online(){
	while true; do
		if timeout 30 bash -c "until ping -q -c3 kria-gmsl.local &>/dev/null; do false; done"
		then
			echo_blue "Connection to DUT OK"
			break
		else
			echo_red "Waiting for board to come online..."
		fi
	done
}
...
ssh_cmd() {
	...
	sshpass -p${PASS} ssh -X -q -t -oConnectTimeout=10 -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -oCheckHostIP=no "$USER"@"$CLIENT" "$CMD"
}
