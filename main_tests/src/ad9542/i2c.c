...
const int16_t ad9542_regs_status[5][2] = {
  ...
  const int16_t ad9542_regs_EEPROM_Inst1[15][2] = {
	...
	{0x2E14,0x00},	
	{0x2E15,0x11},	
	{0x2E16,0x04},  // driver config Ch1; 4 regs from 0x14D7
	{0x2E17,0xD7},	
	{0x2E18,0x14},	
...
int main(void)
{
	...
	for(i = 0; i < 13; i++)	{
		ad9542_set_reg_value(desc, ad9542_regs_output[i][0], ad9542_regs_output[i][1]);
	}
	// IO Update
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);	
	
	// sysclk config
	printf ("SYSCLK Config \n");
	for(i = 0; i < 9; i++)	{
		ad9542_set_reg_value(desc, ad9542_regs_sysclk[i][0], ad9542_regs_sysclk[i][1]);
	...
	// Calibrate SYSCLK
	ad9542_set_reg_value(desc, regGlobal, 0x04);
	// IO Update
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	ad9542_set_reg_value(desc, regGlobal, 0x00);
	// IO Update
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	...
	printf ("APLL Config \n");
	for(i = 0; i < 14; i++)	{
		ad9542_set_reg_value(desc, ad9542_regs_apll[i][0], ad9542_regs_apll[i][1]);
	}
	// IO Update
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	
	// Calibrate APLL0
	ad9542_set_reg_value(desc, regAPLL0Calib, 0x02);
	// IO Update
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	ad9542_set_reg_value(desc, regAPLL0Calib, 0x00);
	// IO Update
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	
	// Calibrate APLL1
	ad9542_set_reg_value(desc, regAPLL1Calib, 0x02);
	// IO Update
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	ad9542_set_reg_value(desc, regAPLL1Calib, 0x00);
	// IO Update
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	...
	// Enable EEPROM write
	ad9542_set_reg_value(desc, regEepromConfifg, 0x01);
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);

	printf ("Download to EEPROM Instruction Set 1 \n");	
	for(i = 0; i < 15; i++) {
		ad9542_set_reg_value(desc, ad9542_regs_EEPROM_Inst1[i][0], ad9542_regs_EEPROM_Inst1[i][1]);		
	}
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	// save to EEPROM
	ad9542_set_reg_value(desc, regEepromSave, 0x01);	
	// wait while EEPROM save in progress
	ad9542_get_reg_value(desc, regEepromStatus, &val);
	printf ("EEPROM STATUS : 0x%x \n",val);	
	while ((val & 0x01) != 0) {
		ad9542_get_reg_value(desc, regEepromStatus, &val);		
	}
	
	printf ("Download to EEPROM Instruction Set 2 \n");		
	for(i = 0; i < 15; i++) {
		ad9542_set_reg_value(desc, ad9542_regs_EEPROM_Inst2[i][0], ad9542_regs_EEPROM_Inst2[i][1]);		
	}
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	// save to EEPROM
	ad9542_set_reg_value(desc, regEepromSave, 0x01);	
	...
	printf ("Download to EEPROM Instruction Set 3 \n");		
	for(i = 0; i < 15; i++) {
		ad9542_set_reg_value(desc, ad9542_regs_EEPROM_Inst3[i][0], ad9542_regs_EEPROM_Inst3[i][1]);		
	}
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	// save to EEPROM
	ad9542_set_reg_value(desc, regEepromSave, 0x01);	
	...
	printf ("Download to EEPROM Instruction Set 4 \n");		
	for(i = 0; i < 15; i++) {
		ad9542_set_reg_value(desc, ad9542_regs_EEPROM_Inst4[i][0], ad9542_regs_EEPROM_Inst4[i][1]);		
	}
	ad9542_set_reg_value(desc, regIoUpdate, 0x01);
	// save to EEPROM
	ad9542_set_reg_value(desc, regEepromSave, 0x01);	
	...
	// Verify if EEPROM fault
	ad9542_get_reg_value(desc, regEepromStatus, &val);
	printf ("EEPROM STATUS : 0x%x \n",val);	
	if ((val & (1 << 2)) == 0x00) 
		printf ("EEPROM SAVE SUCCESS \n");
	else 
		printf ("EEPROM SAVE FAULT : 0x%x \n",val);	
